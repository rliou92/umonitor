			 ...
			 config_setting_t * profile_group)
{
	...
	self->umon_setting.disp_width =
	    config_setting_add(self->umon_setting.disp_group, "width",
			       CONFIG_TYPE_INT);
	self->umon_setting.disp_height =
	    config_setting_add(self->umon_setting.disp_group, "height",
			       CONFIG_TYPE_INT);
	self->umon_setting.disp_widthMM =
	    config_setting_add(self->umon_setting.disp_group, "widthMM",
			       CONFIG_TYPE_INT);
				...
				xcb_randr_output_t * output_p)
{
	...
	if (!self->output_info_reply->connection) {
		...
		self->umon_setting.res_x =
		    config_setting_add(self->umon_setting.res_group, "x",
				       CONFIG_TYPE_INT);
		self->umon_setting.res_y =
		    config_setting_add(self->umon_setting.res_group, "y",
				       CONFIG_TYPE_INT);
		self->umon_setting.pos_group =
		    config_setting_add(self->umon_setting.output_group,
				       "pos", CONFIG_TYPE_GROUP);
		self->umon_setting.pos_x =
		    config_setting_add(self->umon_setting.pos_group, "x",
				       CONFIG_TYPE_INT);
		self->umon_setting.pos_y =
		    config_setting_add(self->umon_setting.pos_group, "y",
				       CONFIG_TYPE_INT);

		fetch_edid(self->cur_output, self->screen_t_p,
			   &edid_string);
		config_setting_set_string(self->umon_setting.edid_setting,
					  edid_string);
		free(edid_string);
		...
		if (self->output_info_reply->crtc) {
			...
			if (*output_p == self->primary_output) {
				...
				config_setting_t *primary_output_setting =
				    config_setting_add(self->
						       umon_setting.output_group,
						       "primary",
						       CONFIG_TYPE_INT);
				config_setting_set_int
				    (primary_output_setting, 1);
...
static void output_info_to_config(save_class * self)
{
	...
	crtc_info_cookie =
	    xcb_randr_get_crtc_info(self->screen_t_p->c,
				    self->output_info_reply->crtc,
				    self->
				    screen_t_p->screen_resources_reply->config_timestamp);
	self->crtc_info_reply =
	    xcb_randr_get_crtc_info_reply(self->screen_t_p->c,
					  crtc_info_cookie,
					  &self->screen_t_p->e);
	...
	if (outputs_length > 1) {
		...
		dup_outputs =
		    xcb_randr_get_crtc_info_outputs(self->crtc_info_reply);
		dup_outputs_setting =
		    config_setting_add(self->umon_setting.mon_group,
		...
		for (i = 0; i < outputs_length; ++i) {
			output_info_cookie =
			    xcb_randr_get_output_info(self->screen_t_p->c,
						      dup_outputs[i],
						      XCB_CURRENT_TIME);
			output_info_reply =
			    xcb_randr_get_output_info_reply
			    (self->screen_t_p->c, output_info_cookie,
			     &self->screen_t_p->e);
	...
	config_setting_set_int(self->umon_setting.pos_x,
			       self->crtc_info_reply->x);
	config_setting_set_int(self->umon_setting.pos_y,
			       self->crtc_info_reply->y);
